window.WebDashboardView = countlyView.extend({
    ...
    renderCommon:function (isRefresh, isDateChange) {
        ...
        if (!isRefresh) {
            ...
            var sort = 3;
            var columns = [
				{ "mData": function(row){var c = (!row["cc"]) ? "Unknown" : row["cc"]; if(c != "Unknown") c = '<div class="flag" style="background-image: url(images/flags/'+ c.toLowerCase() + '.png);"></div>'+c; if(row["cty"] != "Unknown") c += " ("+row["cty"]+")"; return c;}, "sType":"string", "sTitle": jQuery.i18n.map["countries.table.country"], "bSortable":false, "sClass": "break web-15" },
...
$( document ).ready(function() {
    ...
    menu = '<a href="#/analytics/channel" class="item">'+
		'<div class="logo channel"></div>'+
		'<div class="text" data-localize="sidebar.analytics.channel"></div>'+
	'</a>';
	$('#web-type #analytics-submenu').prepend(menu);
